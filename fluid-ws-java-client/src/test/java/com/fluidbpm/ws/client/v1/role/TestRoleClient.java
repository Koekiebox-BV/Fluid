...
public class TestRoleClient extends ABaseLoggedInTestCase {
    ...
    public void testRole_CRUD() {
        ...
        try {
            ...
            Role roleToWipe = roleClient.getRoleByName(roleToCreate.getName());
            roleClient.deleteRole(roleToWipe, true);
            roleToCreate = roleClient.createRole(roleToCreate);
        }
        ...
        TestCase.assertTrue("'Admin Permission' count mismatch. Is "+ roleToCreate.getAdminPermissions(),
        ...
        TestCase.assertTrue(
                ...
                roleToUpdate.getRoleToFormFieldToFormDefinitions() == null ||
                        roleToUpdate.getRoleToFormFieldToFormDefinitions().isEmpty());

        //Role to Form Definition...
        TestCase.assertTrue(
                "UPDATE: 'Role To Form Definitions' must not be set.",
                roleToUpdate.getRoleToFormDefinitions() == null ||
                        roleToUpdate.getRoleToFormDefinitions().isEmpty());
    ...
    @After
    public void destroy() {
        try (RoleClient roleClient = new RoleClient(BASE_URL, ADMIN_SERVICE_TICKET)) {
            Role delMe = roleClient.getRoleByName(TestStatics.Update.ROLE_NAME);
            roleClient.deleteRole(delMe, true);
        } catch (FluidClientException fce) {

        }

        try (RoleClient roleClient = new RoleClient(BASE_URL, ADMIN_SERVICE_TICKET)) {
            Role delMe = roleClient.getRoleByName(TestStatics.Create.ROLE_NAME);
            roleClient.deleteRole(delMe, true);
        } catch (FluidClientException fce) {
